    ...
    <div class="bg-white rounded-lg shadow-lg p-6 text-center">
        ...
        <form method="POST" action="{{route('profile.delete.account')}}">
            ...
            <div class="mt-4 flex justify-center">
                <input type="password" name="password" placeholder="{{ __('profile.delete-account-password') }}" class="border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 rounded-md shadow-sm mt-1 block w-3/4" required>
            </div>

            <div class="mt-4 flex justify-center space-x-4">
                <!-- Close Button -->
                <button onclick="closeModal()" class="mt-4 px-4 py-2 bg-gray-400 text-white rounded">
                    {{__('lang.close')}}
                </button>
                <button type="submit" class="mt-4 px-4 py-2 bg-red-600 text-white rounded-md ml-3">
