    ...
    @if($approval != "Approved" || Auth::user()->role == "Admin" || Auth::user()->role == "Teacher")
        <div class="flex flex-col justify-end">
            @if(request()->routeIs('form.detail'))
                <button class="relative flex items-center justify-center h-12 w-12 mt-2 mb-2 mx-auto shadow-lg bg-gray-800 text-red-500 hover:bg-red-500 hover:text-gray-800 rounded-3xl hover:rounded-xl transition-all duration-300 ease-linear group cursor-pointer" href="{{route('progress')}}"">
            ...
            @if(request()->routeIs('course.assign'))
                <form action="{{route('custom.assign', ['id' => $id, 'courseid' => $courseid])}}" method="POST" class="inline-block">
                    @csrf
                    <button type="submit"  class="relative flex items-center justify-center h-12 w-12 mt-2 mb-2 mx-auto shadow-lg bg-gray-800 text-blue-500 hover:bg-blue-500 hover:text-gray-800 rounded-3xl hover:rounded-xl transition-all duration-300 ease-linear group cursor-pointer">
