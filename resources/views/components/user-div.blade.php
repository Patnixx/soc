...
<a data-role="{{$role}}" class="{{$aclass}} grid grid-cols-3 sm:grid-cols-8 w-full h-auto px-4 py-2 border-b border-gray-300 dark:border-gray-700 hover:bg-gray-100 dark:hover:bg-gray-800 transition-all bg-gradient-to-br from-transparent via-transparent">
    ...
    <div class="flex flex-col sm:flex-row items-center justify-end text-sm truncate sm:space-x-4">
        <form action="{{route('users.edit', $id)}}" method="GET" class="flex items-center justify-center self-center">
            @csrf
            <button type="submit">
                <i class="{{$iclass}} bi bi-pencil hover:text-gray-500 text-yellow-600 dark:hover:text-gray-400 dark:text-yellow-500"></i>
            </button>
        </form>
        <form action="{{route('users.delete', $id)}}" method="POST" class="flex items-start justify-start self-center">
            @csrf
            <button type="submit">
