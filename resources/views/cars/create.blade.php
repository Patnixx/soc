...
<div class="container mx-auto max-w-[70%] mt-32 rounded-lg shadow-lg bg-white dark:bg-gray-900 p-6 my-8 transition-all duration-300 ease-linear">
    ...
    <form action="{{ route('cars.store') }}" method="POST" enctype="multipart/form-data" class="grid grid-cols-2 gap-6 mb-6">
        @csrf
        <div class="left">
            <div class="flex flex-col space-y-2 mb-2">
                <label for="brand" class="text-sm font-medium dark:text-white text-gray-900">{{__('car.brand')}}</label>
                <x-input-div :name="'brand'" :type="'text'" :placeholder="'brand'" :id="'brand'" :value="''" :icon="'bi bi-car-front'"/>
            </div>
            <div class="flex flex-col space-y-2 mb-2">
                <label for="model" class="text-sm font-medium dark:text-white text-gray-900">{{__('car.model')}}</label>
                <x-input-div :name="'model'" :type="'text'" :placeholder="'model'" :id="'model'" :value="''" :icon="'bi bi-car-front'"/>
            </div>
            <div class="flex flex-col space-y-2 mb-2">
                <label for="year" class="text-sm font-medium dark:text-white text-gray-900">{{__('car.year')}}</label>
                <x-input-div :name="'year'" :type="'text'" :placeholder="'year'" :id="'year'" :value="''" :icon="'bi bi-calendar'"/>
            </div>
            <div class="flex flex-col space-y-2 mb-2">
                <label for="horsepower" class="text-sm font-medium dark:text-white text-gray-900">{{__('car.hp')}}</label>
                <x-input-div :name="'horsepower'" :type="'text'" :placeholder="'horsepower'" :id="'horsepower'" :value="''" :icon="'bi bi-speedometer'"/>
            </div>
        </div>
        <div class="right">
            <div class="flex flex-col space-y-2 mb-2">
                <label for="cubage" class="text-sm font-medium dark:text-white text-gray-900">{{__('car.engine')}}</label>
                <x-input-div :name="'cubage'" :type="'text'" :placeholder="'cubage'" :id="'cubage'" :value="''" :icon="'bi bi-gear-wide-connected'"/>
            </div>
            <div class="flex flex-col space-y-2 mb-2">
                <label for="gearbox" class="text-sm font-medium dark:text-white text-gray-900">{{__('car.gearbox')}}</label>
                <x-input-div :name="'gearbox'" :type="'text'" :placeholder="'gearbox'" :id="'gearbox'" :value="''" :icon="'bi bi-gear'"/>
            </div>
            <div class="flex flex-col space-y-2 mb-2">
                <label for="drive" class="text-sm font-medium dark:text-white text-gray-900">{{__('car.drive')}}</label>
                <x-input-div :name="'drive'" :type="'text'" :placeholder="'drive'" :id="'drive'" :value="''" :icon="'bi bi-arrow-repeat'"/>
            </div>
            <div class="flex flex-col space-y-2 mb-2">
                <label for="mileage" class="text-sm font-medium dark:text-white text-gray-900">{{__('car.mileage')}}</label>
