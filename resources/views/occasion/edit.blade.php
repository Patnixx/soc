...
@endif
@if($event->type == 'VP')
    @section('title', ''.__('occasions.edit-vp').'')
@endif
@if($event->type == 'KPP')
    @section('title', ''.__('occasions.edit-kpp').'')
@endif
@if($event->type == 'Exam')
    @section('title', ''.__('occasions.edit-exam').'')
@endif
@section('content')
    @if($event->type == 'Theory')
                ...
                <div class="flex flex-col space-y-2">
                    ...
                    <select required name="user" id="user" class="w-full px-4 py-2 border rounded-lg shadow-sm focus:ring focus:ring-blue-300">
                        @foreach($students as $student)
                            <option value="{{$student->user_id}}">{{$student->user->f_name}} {{$student->user->l_name}}</option>
                        @endforeach
                    </select>
                </div>
                <button 
    ...
    @if($event->type == 'VP')
        <div class="flex flex-col items-center justify-center min-h-screen px-4 sm:px-6 lg:px-8">
            <form action="{{ route('events.update', $event->id) }}" method="post" class="w-full max-w-xl bg-white dark:bg-gray-900 shadow-lg rounded-lg p-6 space-y-4 transition-all duration-300 ease-linear">
                @csrf
                <h2 class="text-2xl font-semibold dark:text-white text-gray-900 text-center">{{ __('occasions.edit-vp') }}</h2>
                <div class="flex flex-col space-y-2">
                    <label for="name" class="text-sm font-medium dark:text-white text-gray-900">{{ __('occasions.name') }}:</label>
                    <input 
                        type="text" 
    ...
    @if($event->type == 'KPP')
        <div class="flex flex-col items-center justify-center min-h-screen px-4 sm:px-6 lg:px-8">
            <form action="{{ route('events.update', $event->id) }}" method="post" class="w-full max-w-xl bg-white dark:bg-gray-900 shadow-lg rounded-lg p-6 space-y-4 transition-all duration-300 ease-linear">
                @csrf
                <h2 class="text-2xl font-semibold dark:text-white text-gray-900 text-center">{{ __('occasions.edit-kpp') }}</h2>
                <div class="flex flex-col space-y-2">
                    <label for="name" class="text-sm font-medium dark:text-white text-gray-900">{{ __('occasions.name') }}:</label>
                    <input 
                        type="text" 
    ...
    @if($event->type == 'Exam') 
        <div class="flex flex-col items-center justify-center min-h-screen px-4 sm:px-6 lg:px-8">
            <form action="{{ route('events.update', $event->id) }}" method="post" class="w-full max-w-xl bg-white dark:bg-gray-900 shadow-lg rounded-lg p-6 space-y-4 transition-all duration-300 ease-linear">
                @csrf
                <h2 class="text-2xl font-semibold dark:text-white text-gray-900 text-center">{{ __('occasions.edit-exam') }}</h2>
                <div class="flex flex-col space-y-2">
                    <label for="name" class="text-sm font-medium dark:text-white text-gray-900">{{ __('occasions.name') }}:</label>
                    <input 
                        type="text" 
