...
<div class="flex flex-col items-center justify-center min-h-screen px-4 sm:px-8">
    <form action="{{ route('events.store.exam') }}" method="post" class="w-full max-w-lg sm:max-w-xl bg-white dark:bg-gray-900 shadow-lg rounded-lg p-6 space-y-4 transition-all duration-300 ease-linear">
        @csrf
        <h2 class="text-2xl font-semibold dark:text-white text-gray-900 text-center">{{ __('occasions.create-exam') }}</h2>
