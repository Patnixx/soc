    ...
    <div class="w-full max-w-sm sm:w-112 h-auto sm:h-144 bg-white dark:bg-gray-900 transition-all duration-300 ease-linear flex flex-col items-center justify-center shadow-lg px-6 py-8 sm:p-8 rounded-3xl">
        ...
        <form action="{{ route('custom.login')}}" method="post" class="w-full flex flex-col gap-6">
            ...
            <div class="form-group relative w-full">
                <input type="password" name="password" id="pass" class="w-full py-3 pl-10 pr-10 dark:bg-gray-800 dark:text-white bg-slate-200 rounded-lg focus:ring-1 dark:focus:ring-m-blue focus:ring-gray-900 transition-all duration-300 ease-linear"placeholder="{{ __('auth.pass-placeholder') }}" value="" required>
            ...
            <button type="submit" class="dark:bg-m-blue dark:text-white bg-m-blue text-gray-900 hover:bg-m-red hover:text-white py-3 px-6 rounded-lg font-bold dark:hover:bg-m-darkblue transition duration-300 ease-linear">{{__('auth.login')}}</button>
