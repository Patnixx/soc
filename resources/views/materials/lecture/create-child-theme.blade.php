...
<div class="flex flex-col items-center justify-center min-h-screen p-4 sm:p-6">
    <form action="{{ route('childlecture.assign', $syllab) }}" method="post" 
        class="w-full sm:w-3/4 max-w-lg bg-white dark:bg-gray-900 shadow-lg rounded-lg p-6 space-y-4 transition-all duration-300 ease-linear">
        @csrf
        ...
        <button 
            ...
            hover:bg-m-red hover:text-white dark:hover:bg-m-darkblue">
            {{ __('materials.assign-sub-theme') }}
        </button>
    </form>
    <a href="{{ route('lecture.view', $syllab) }}" 
        ...
        <span class="absolute w-auto p-2 m-2 min-w-max left-14 sm:left-auto sm:right-14 rounded-md shadow-md text-white bg-gray-900 text-xs font-bold 
