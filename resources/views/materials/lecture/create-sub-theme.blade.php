...
<div class="flex flex-col items-center justify-center min-h-screen p-4 sm:p-6">
    <form action="{{route('sublecture.store', $syllab)}}" method="post" class="w-full max-w-md sm:max-w-lg bg-white dark:bg-gray-900 shadow-lg rounded-lg p-4 sm:p-6 space-y-4 transition-all duration-300 ease-linear">
        @csrf
    ...
    <a href="{{route('lecture.view', $syllab)}}" class="relative flex items-center justify-center h-12 w-12 mt-4 shadow-lg bg-slate-200 dark:bg-gray-800 text-red-500 hover:bg-red-500 hover:text-gray-800 dark:hover:text-white rounded-3xl hover:rounded-xl transition-all duration-300 ease-linear group cursor-pointer">
