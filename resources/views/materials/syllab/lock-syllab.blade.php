...
<div class="flex flex-col items-center justify-center min-h-screen p-6">
        ...
        <div class="space-y-4">
            @foreach ($syllab as $relation)
                <div class="items-center justify-between border-b pb-2 flex flex-col sm:flex-row sm:items-center sm:space-x-4 gap-4">
                    <span class="text-sm font-medium dark:text-white text-gray-900 sm:w-1/3">
                        {{ $relation->course->name }}
                    </span>
                    <label class="flex items-center justify-start sm:justify-center space-x-2 sm:w-1/3">
                        <input 
                            type="checkbox" 
                    ...
                    <a class="flex items-center justify-center sm:w-1/3" href="{{route('syllab.removeCourse', ['courseId' => $relation->id, 'syllabId' => $section->id])}}">
    ...
    <div class="flex flex-row items-center justify-center mt-4 space-x-2 flex-wrap">
        ...
        <a 
            class="relative flex items-center justify-center h-12 w-12 mt-2 mb-2 shadow-lg bg-m-blue dark:bg-gray-800  text-gray-900 dark:text-m-blue hover:bg-m-red dark:hover:bg-m-darkblue hover:text-white dark:hover:text-white rounded-3xl hover:rounded-xl transition-all duration-300 ease-linear group cursor-pointer" 
